...
public class PANVerificationService {
    @Autowired
    PANVerificationResponseRepo verificationResponseRepo;
    @Autowired
    PANRequestRepo panRequestRepo;

    public PANVerificationResponse getPANVerDetFromAPI(PANRequest panRequest)  {
        System.out.println("inside verification service");
        panRequestRepo.save(panRequest);
        PANVerificationResponse panStatus=null;
        String apiUrl = Config.kycUrl+"/kyc/external/panDataFetch";
...
//                .build();
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            if (response.isSuccessful()) {
                ...
                System.out.println("responseBody.toString() "+responseBody);
                panStatus=PANConverter.convertJsonTOPANRespnse(responseBody.source().readUtf8());
                System.out.println("panStatus "+panStatus.getData());
                verificationResponseRepo.save(panStatus);
